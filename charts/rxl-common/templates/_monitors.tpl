...
*/}}
{{- define "rxl-common.monitorSampleLimit" }}
{{- default 25000 .Values.monitor.sampleLimit }}
{{- end }}

{{/*

The monitoring port needs to be exposed on a Pod in order for Prometheus
to scrape it. We automatically open that port up as long as
...
*/}}
{{- define "rxl-common.monitorPodPorts" }}
...
metadata:
  ...
  labels:
    ...
    {{- end }}
spec:
  sampleLimit: {{ include "rxl-common.monitorSampleLimit" . }}
  selector:
    matchLabels:
...
*/}}
{{- define "rxl-common.serviceMonitor" }}
{{- if .Values.monitor.enabled }}
apiVersion: monitoring.coreos.com/v1
kind: ServiceMonitor
metadata:
...
{{- define "rxl-common.monitorRules" }}
{{- if .Values.monitor.enabled }}
apiVersion: monitoring.coreos.com/v1
kind: PrometheusRule
metadata:
  name: {{ include "rxl-common.fullname" . }}-monitor-rules
  {{- with .Values.monitor.annotations }}
  annotations:
    {{- toYaml . | nindent 4 }}
  {{- end }}
...
spec:
  groups:
  - name: {{ include "rxl-common.fullname" . }}.monitorRules
    rules:
      - record: pod_monitor_sample_limit
        expr: {{ (include "rxl-common.monitorSampleLimit" .) | quote }}
        labels:
          namespace: {{ .Release.Namespace }}
          name: {{ include "rxl-common.name" . }}
